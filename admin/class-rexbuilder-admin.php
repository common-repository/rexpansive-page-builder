	...
	public function enqueue_scripts( $hook ) {
		...
		if( $this->builder_active_on_this_post_type( $page_info ) ) {
			...
			if( isset( $_GET['rexlive'] ) && 'true' == $_GET['rexlive'] ) {
				...
				wp_enqueue_script( 'rexlive-section-nav-label-modal', REXPANSIVE_BUILDER_URL . 'admin/js/builderlive/Rexlive_Section_NavLabel_Modal.js', array( 'jquery' ), null, true );
				...
				wp_enqueue_script( 'Rexlive-SectionOrderChanged', REXPANSIVE_BUILDER_URL . 'admin/js/builderlive/Rexlive_SectionOrderChanged_Modal.js', array( 'jquery' ), null, true );
				...
				wp_enqueue_script( 'Rexlive-Element-Import', REXPANSIVE_BUILDER_URL . 'admin/js/builderlive/Rexlive_Form_Import.js', array( 'jquery' ), null, true );
...
<div class="col rex-slider__slide rex-modal-content__modal-area__row" data-slider-slide-id="<?php echo $j ?>" data-block_type="slide">
	...
	*/
	...
	public function rex_delete_rexelement() {
		...
		self::separate_forms_in_posts($_POST['element_id'], $_POST['postID']);
	...
	public function rex_delete_rexbutton() {
		...
		// @todo separate other buttons present on pages
		$button_ids = json_decode( stripslashes( get_option( '_rex_buttons_ids' ) ), true );

		self::separate_buttons_in_posts( $_POST['button_id'], $button_ids );
	...
	private static function separate_buttons_in_posts( $rexButtonID, &$button_ids ) {
		...
		$originalButton = stripslashes( get_option( "_rex_button_{$rexButtonID}_html" ) );
		...
		if ( count( $posts_with_buttons ) > 0 ) {
			...
			foreach( $posts_with_buttons as $post ) {
				...
				// manipulate post buttons ids in page
				$buttons_ids_in_page = json_decode( get_post_meta( $post_id, '_rexbuilder_buttons_ids_in_page', true ), true );
				$b_index = array_search( $rexButtonID, $buttons_ids_in_page );
				array_splice( $buttons_ids_in_page, $b_index, 1 );
