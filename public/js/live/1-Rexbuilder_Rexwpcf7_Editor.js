var Rexbuilder_Rexwpcf7_Editor = (function ($) {
	...
	function addRow(formID, $rowToAdd, numberRowBefore) {
		...
		var columnsAdded = Array.prototype.slice.call($rowToAdd.get(0).getElementsByClassName('wpcf7-column'));
		var newRowNumber = numberRowBefore + 1;

		columnsAdded.forEach(function (clonedColumn, i) {
			if (Rexbuilder_Util.hasClass(clonedColumn, 'with-button')) return;

			_fixClonedFieldsNames(clonedColumn, formID, newRowNumber, i + 1);
	...
	function addClonedColumnRow(formID, clonedColumnNumber, numberRowBefore) {
		...
		wrapButtons();
		_fixClonedFieldsNames(clonedColumn, formID, newRowNumber, clonedColumnNumber);
	}

	/**
	 * Changes the fields' names to make them unique,
	 * needed to distinguish them in the backend.
	 * @param	{Element}				clonedColumn
	 * @param	{String}				formID
	 * @param	{String|Number}	rowNumber
	 * @param	{String|Number}	columnNumber
	 * @since	2.0.5
	 */
	function _fixClonedFieldsNames(clonedColumn, formID, rowNumber, columnNumber) {
		// Field name handling
		var nameData = _getClonedFieldNewInfo(clonedColumn);
	...
	/**
	 * Given the cloned form column, returns an object containing
	 * info about it. The newName property of the returned object
	 * is intended to be unique. The returned value === null means
	 * that the cloned field is a submit, which doesn't have a name.
	 * @param		{Element}			clonedColumn
	 * @returns	{Object|null}	Contains:
	 * 													- inputType: e.g. 'text', 'submit', 'file', ...
	 * 													- oldName: then input name before the cloning
	 * 													- newName: then new unique input name
	 * @since		2.0.5
	 */
	function _getClonedFieldNewInfo(clonedColumn) {
		...
		// because there could be already cloned inputs with the "-rexclone-{x}" suffix
		...
		// Input name to test for getting the highest clone number
		var testName;

		// Suffix for the "-rexclone-" string attached to the original input name
		// Needed when an input is cloned several times
		var highestCloneNumber = 1;

		inputsWithSameName.forEach(function (input) {
			testName = input.getAttribute('name').match(cloneRE);

			if (testName) {
				highestCloneNumber = parseInt(testName[2]) > highestCloneNumber ? parseInt(testName[2]) : highestCloneNumber;
			}
		});
		...
		if (wasCloned) {
			newName = oldName.replace(cloneRE, '-rexclone-' + highestCloneNumber);
		} else {
			newName = oldName + '-rexclone-' + highestCloneNumber;
		}
	...
	/**
	 * Replaces the old field name occurrencies in the cloned db column
	 * containing the shortcode.
	 * @param	{String}	formID
	 * @param	{Object}	options	Contains:
	 * 														- newRowNumber
	 * 														- newColumnNumber
	 * 														- oldName: field name to replace
	 * 														- newName
	 * @since	2.0.5
	 */
	function _fixClonedFieldsDB(formID, options) {
		...
		var shortcode = dbColumnContent.innerHTML;

		if (-1 !== shortcode.indexOf(oldName)) {
			dbColumnContent.innerHTML = shortcode.replace(oldName, newName);
		} else {
			...
			case 'wpcf7-placeholder':
				switch (inputType) {
					...
					case 'textarea':
						...
						break;
					case 'select':
						$formColumns.find('.wpcf7-' + inputType + ' option[disabled]').text(newValue);
						break;
					...
					case 'select':
						...
						formColumns.forEach(function (column) {
							newValue.fields.forEach(function (newVal, i) {
								actualOption = column.querySelectorAll('.wpcf7-select option:not([disabled])')[i];
	...
	function _saveNewField(insertionPoint, fieldShortcode) {
		...
		$columnToUpdateDB.removeClass('with-button').append(fieldShortcode);
	}
	...
	function _fixRowNumbersAndClasses($forms) {
		...
		$forms.find('.wpcf7-row').each(function (index) {
			var $formRow = $(this);
	...
	function _getName(formControl) {
		...
		type = formControl.className.match(/wpcf7-(text|number|textarea|select|submit|acceptance|radio|file)/);
		if (!type) return;
