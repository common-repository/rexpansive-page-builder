...
var TextEditor = (function ($) {
	 ...
	 */
	var addElementToTextEditor = function (textWrap) {
	...
	var openTextGradientColor = function ($elem) {
	...
	var ColorPickerExtension = MediumEditor.extensions.button.extend({
		...
		init: function () {
			...
			this.button.innerHTML =
				"<span class='meditor-color-picker'><span class='meditor-color-picker__placeholder'>P</span></span><span class='meditor-color-picker--preview'></span>";
	...
	var TextTagExtension = MediumEditor.extensions.button.extend({
		...
		init: function () {
			...
			this.list_element.innerHTML =
				"<div class='medium-editor-action' data-tag-action='append-h1'>h1</div><div class='medium-editor-action' data-tag-action='append-h2'>h2</div><div class='medium-editor-action' data-tag-action='append-h3'>h3</div><div class='medium-editor-action' data-tag-action='append-h4'>h4</div><div class='medium-editor-action' data-tag-action='append-h5'>h5</div><div class='medium-editor-action' data-tag-action='append-h6'>h6</div><div class='medium-editor-action' data-tag-action='append-p'>p</div>";
		...
		activateListButtons: function () {
			if ('' != this.action_active) {
				this.list_element
					.querySelector('.medium-editor-action[data-tag-action="' + this.action_active + '"]')
					.classList.add('medium-editor-button-active');
	...
	var FormattingTagExtension = MediumEditor.extensions.button.extend({
		...
		activateListButtons: function () {
			for (var i = 0; i < this.action_active.length; i++) {
				this.list_element
					.querySelector('.medium-editor-action[data-tag-action="' + this.action_active[i] + '"]')
					.classList.add('medium-editor-button-active');
				if (0 == i) {
	...
	var ContentBlockPositionExtension = MediumEditor.extensions.button.extend({
		...
		handleClick: function (event) {
			...
			$(this.base.getFocusedElement())
				...
				.find('.edit-block-content-position')
				.trigger('click');
		}
	});
	...
	var RexButtonExtension = MediumEditor.Extension.extend({
		...
		handlePositionToolbar: function (event) {
			...
			if (this.insideRexButton(element)) {
				...
				$toolbar
					.find('button.medium-editor-action:not(.hide-tool-rexbutton)')
					.first()
		...
		handleBlur: function (event) {
			if (
				$(event.target).parents('.rex-button-wrapper').length == 0 &&
				$(event.target).parents('.rexbutton-tools').length == 0
			) {
		...
		placeRexbuttonToolbox: function () {
			...
			this.rexbuttonToolsTop.style.left =
				targetCoords.left + (targetCoords.width - this.rexbuttonToolsTop.offsetWidth) / 2 + 'px';
			...
			this.rexbuttonToolsBottom.style.left =
				targetCoords.left + (targetCoords.width - this.rexbuttonToolsBottom.offsetWidth) / 2 + 'px';
		...
		handleEventKeyDown: function (event, target) {
			...
			if (
				MediumEditor.util.isKey(event, this.keyCode.BACKSPACE) &&
				(window.getSelection().focusOffset == 0 || window.getSelection().focusOffset == 1)
			) {
				if (
					//cursor is at the beginning of the element
	...
	var InsertMediaExtension = MediumEditor.Extension.extend({
		...
		handleBlur: function (event) {
			if (
				$(event.target).parents('#me-edit-inline-image-toolbar').length == 0 &&
				$(event.target).parents('#me-edit-inline-svg-toolbar').length == 0 &&
				!$(event.target).is('.me-insert-embed__value') &&
				0 == $(event.target).parents('.me-insert-embed').length
			) {
				this.mediaBtn.style.display = 'none';
		...
		placeMediaBtnBottomTextEditor: function () {
			...
			this.mediaBtn.style.top =
				window.scrollY + targetCoords.top + targetCoords.height - this.mediaBtn.offsetHeight + 'px';
		},
		...
		placeMediaBtn: function ($wrapper) {
			...
			var targetCoords = $content_wrap[0].getBoundingClientRect();
			this.mediaBtn.style.left =
				window.scrollX + targetCoords.left + targetCoords.width - this.mediaBtn.offsetWidth - 15 + 'px';
			this.mediaBtn.style.top =
				window.scrollY + targetCoords.top + targetCoords.height - this.mediaBtn.offsetHeight - 15 + 'px';
		},
		...
		traceInput: function (event) {
			...
			if ('click' == event.type) {
				...
				if ('IMG' == event.target.nodeName) {
				...
				} else if ('SPAN' == event.target.nodeName && 'overlay-status-for-video-inline' == event.target.className) {
					// Check if the clicked object is an <span> with the class "overlay-status-set-active". -A
					// if a mirror already exists but doesn't have handles, destroy it
					...
					('USE' == event.target.nodeName.toUpperCase() && '' !== event.target.getAttribute('xlink:href')) ||
					('SVG' === event.target.nodeName.toUpperCase() && 'I' === event.target.parentNode.nodeName.toUpperCase())
				) {
					var target;
					...
					// If no positive results is received.
					this.hideAllToolbars();
				}
			}
		},
		...
		handleImageInsertReplace: function (event) {
			var imgHTML =
				'<img class="wp-image-' +
				event.imgData.idImage +
				' ' +
				event.imgData.align +
				'" data-image-id="' +
				event.imgData.idImage +
				'" src="' +
				event.imgData.urlImage +
				'" alt="" width="' +
				event.imgData.width +
				'" height="' +
				event.imgData.height +
				'">';
		...
		handleImageEdit: function (event) {
			...
			if ($el.hasClass('me-image-align-left')) {
				...
				if (
					this.mirrorResize.classList.contains('ui-resizable') &&
					this.mirrorResize.parentElement.classList.contains('ui-wrapper')
				) {
		...
		setActiveButtonsImgEditToolbar: function (imageTarget) {
			...
			$el.each(function () {
				...
				if ($(this).hasClass('me-image-inline-photoswipe')) {
					if (
						'undefined' !== typeof imageTarget.getAttribute('inline-photoswipe') &&
						imageTarget.getAttribute('inline-photoswipe') == 'true'
					) {
						this.classList.add('medium-editor-button-active');
					} else {
						this.classList.remove('medium-editor-button-active');
		...
		imageResizableEnable: function () {
			...
			$mirrorResize.resizable({
				...
				create: function (event, ui) {
					...
					$wrapper
						.find('.ui-resizable-e')
						.append('<span class="img-resize-handle img-resize-handle-e" data-axis="e"></span>');
					$wrapper
						.find('.ui-resizable-se')
						.append('<span class="img-resize-handle img-resize-handle-se" data-axis="se"></span>');
					$wrapper
						.find('.ui-resizable-s')
						.append('<span class="img-resize-handle img-resize-handle-s" data-axis="s"></span>');
		...
		placeEditImgToolbar: function () {
			...
			this.imageEditToolbar.style.left =
				targetCoords.left + (targetCoords.width - this.imageEditToolbar.offsetWidth) / 2 + 'px';
			this.imageEditToolbar.style.top =
				window.scrollY + targetCoords.top - this.imageEditToolbar.offsetHeight - 8 + 'px';
		},

		placeEditInlineSVGToolbar: function () {
			var targetCoords = this.traceSVG.getBoundingClientRect();
			this.inlineSVGEditToolbar.style.left =
				targetCoords.left + (targetCoords.width - this.inlineSVGEditToolbar.offsetWidth) / 2 + 'px';
			this.inlineSVGEditToolbar.style.top =
				window.scrollY + targetCoords.top - this.inlineSVGEditToolbar.offsetHeight - 8 + 'px';
		},
    ...
    */
      ...
      console.log("OUTPUT:", TransferVideoUrl);
		...
		handleInlineSVGInsertReplace: function (event) {
			...
			var icon_html =
				'<i class="' + event.svg_class + '"><svg><use xlink:href="#' + event.svg_ID + '"></use></svg></i>';
	...
	function _addEditorInstanceEvents() {
		editorInstance.subscribe('editableInput', _handleEditorEditableInput);
		editorInstance.subscribe('editableKeydown', _handleEditorEditableKeydown);
		editorInstance.subscribe('editablePaste', _handleEditorEditablePaste);
	 ...
	 * @since	2.0.8
	 */
	function _addElementsManipulationListeners() {
	...
	var destroyMediumEditor = function () {
	...
	var createEditor = function () {
			...
			toolbar: {
				buttons: [
					...
					'formattingTags',
					{
						...
						formSaveLabel:
							'<span class="tool-button tool-button--inline"><i class="l-svg-icons"><svg><use xlink:href="#A006-Save"></use></svg></i></span>',
						formCloseLabel:
							'<span class="tool-button tool-button--inline tool-button--black"><i class="l-svg-icons"><svg><use xlink:href="#Z003-Close"></use></svg></i></span>'
					},
					'headingTags',
					'justifyDropdown',
					// {
		...
		_addElementsManipulationListeners();
	};
	 ...
	 */
	var removePlaceholder = function ($textWrap) {
	...
	function _handleEditorEditableInput(event, elem) {
		var sectionIsModel = $(elem).parents('.rexpansive_section').hasClass('rex-model-section');

		Rexbuilder_Util_Editor.builderEdited(sectionIsModel);
	}

	function _handleEditorEditableKeydown(event, editableElement) {
		var isEscapeKey = MediumEditor.util.isKey(event, MediumEditor.util.keyCode.ESCAPE);

		if (isEscapeKey) {
			_unfocusTextWrap(editableElement /* , event */);
		}
	}

	function _handleEditorEditablePaste(event, editableElement) {
	 ...
	 * @since	2.0.9
	 */
	function _unfocusTextWrap(textWrap /* , event */) {
		// If the user hits escape, toggle the data-allow-context-menu attribute (OLD COMMENT)
	...
	return {
		...
		addElementToTextEditor: addElementToTextEditor,
		destroyMediumEditor: destroyMediumEditor,
